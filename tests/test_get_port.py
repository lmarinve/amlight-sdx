...
class TestGetPort:
    ...
    def test_get_port_incorrect_type_empty(self):
        ...
        with pytest.raises(Exception) as valueerror:
            parse_topo.get_port(node="", interface="", topology_name="Daniela.net")
    ...
    def test_get_port_incorrect_type_none(self):
        ...
        with pytest.raises(Exception) as typeError:
            parse_topo.get_port(node=None, interface=None, topology_name="Daniela.net")
    ...
    def test_get_port_incorrect_type_int(self):
        ...
        with pytest.raises(Exception) as typeError:
            parse_topo.get_port(node=1, interface=2, topology_name="Daniela.net")
        assert str(typeError.typename) == "TypeError"

    def test_get_port_enabled_interface_case(self):
        """ Test that you get the proper value when interface value == disabled"""
        result = parse_topo.get_port(node=helper._node, interface=helper._interface, topology_name="Daniela.net")
    ...
    def test_get_port_correct_attributes_types(self):
        ...
        result = parse_topo.get_port(node=helper._node, interface=helper._interface, topology_name="Daniela.net")
        print(result)
        assert result == {'id': 'urn:sdx:port:Daniela.net:s1:1', 'name': 's1-eth1',
                          'node': 'urn:sdx:node:Daniela.net:s1', 'type': '10GE', 'nni':'False',
